...
RaytracedRenderer::RaytracedRenderer(size_t ns_aa,
  ...
  pt->max_backward_depth = max_backward_depth;
...
void RaytracedRenderer::start_raytracing() {
  ...
  if (pt->max_backward_depth > 0)
  {
    for (SceneObjects::SceneLight* light : pt->scene->lights)
    {
      int num_light_samples = pt->ns_area_light;
      ...
      {
        pt->raytrace_light(*light, num_light_samples);
      }
    }
    for (int i = 1; i < pt->max_backward_depth; i++)
    {
      for (int j = pt->bdptBuffer.size() - 1; j >= 0; j--)
      {
        if (j != i - 1)
        {
          break;
        }
        pt->raytrace_bdpt_point(pt->bdptBuffer[j]);
      }
    }
  }
...
void RaytracedRenderer::key_press(int key) {
  ...
  case ',': case '<':
    ...
    break;
  case '9': case '0':
    if (pt->max_backward_depth) pt->max_backward_depth--;
    fprintf(stdout, "[PathTracer] Max ray depth decreased to %zu.\n", pt->max_backward_depth);
    break;
  case 'h': case 'H':
