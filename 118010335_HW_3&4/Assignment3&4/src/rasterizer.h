...
namespace CGL
{
  ...
  typedef enum AntiAliasingMethod { AA_SSAA = 0, AA_MSAA = 1, AA_FXAA = 2 } AntiAliasingMethod;
  ...
  public:
    ...
    virtual unsigned int get_aam() = 0;
    virtual void set_aam(AntiAliasingMethod aa) = 0;
  ...
  private:
    ...
    AntiAliasingMethod aam;

    // Parameter used to identify "edge" in FXAA
    float fxaa_threshold;
    // Parameter used to mix pixels in FXAA
    // Range: [0, 0.5]
    float fxaa_rate;
    ...
    std::vector<uint> subpixel_count_buffer;
  ...
  public:
    RasterizerImp(PixelSampleMethod psm, LevelSampleMethod lsm, AntiAliasingMethod aam,
    ...
    unsigned int get_aam() { return aam; }
    void set_aam(AntiAliasingMethod aa) { aam = aa; 
