...
void DrawRend::init() {
  ...
  software_rasterizer = new RasterizerImp(psm, lsm, aam, width, height, sample_rate);
}
...
static const string aa_strings[] = { "SSAA", "MSAA", "FXAA" };
std::string DrawRend::info() {
  stringstream ss;
  stringstream sample_method;
  sample_method << level_strings[lsm] << ", " << pixel_strings[psm] << ", " << aa_strings[aam];
...
void DrawRend::keyboard_event(int key, int event, unsigned char mods) {
  ...
  case 'I':
    ...
    std::cout << info() << " " << "Time per frame: " << dur << "ms" << std::endl;
  ...
  case 'Z':
    ...
    // toggle SSAA / MSAA
  case 'A':
    aam = (AntiAliasingMethod)((aam + 1) % 3);
    software_rasterizer->set_aam(aam);
    redraw();
    break;
